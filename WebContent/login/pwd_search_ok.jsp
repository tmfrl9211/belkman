...
<%
   ...
   if(!email.equals(""))
   {
	   sql="select * from member where userid='"+userid+"' and name='"+name+"' and email='"+email+"'";
	   chk=0;
   }
   else if(!hphone.equals(""))
       {
	     sql="select * from member where userid='"+userid+"' and name='"+name+"' and hphone='"+hphone+"'";
	     chk=1;
       }
       else
...
<body>
  ...
  <div id=right>
    <section class=pwd_search_ok_section>
      <div id=first> 비밀번호 찾기 </div>
      
      <div id=search_main align=center>
        <div id=sub> 비밀번호 찾기 </div>
        <div id=sub> 고객님 비밀번호 찾기가 완료되었습니다 </div>
        <div id=sub>
            <div id=sub_first> 
              저희 쇼핑몰을 이용해 주셔서 감사합니다. <br>
              다음정보로 가입된 아이디의 비밀번호가 총 1개 있습니다.
            </div>
            <div id=sub_second>
         ...
         <%=sel%> : <%=val%> <p>
                   <input type=radio checked> <%=rs.getString("pwd")%> (개인회원, <%=rs.getString("writeday")%>) <p>
                   고객님 즐거운 쇼핑하세요!!
